...
// Separate (heavier) pass for current club, so it can't reduce base coverage.
async function clubChunk(chunk) {
  const values = chunk.map((id) => `wd:${id}`).join(' ')
  const query = `SELECT ?item ?clubLabel WHERE {
    VALUES ?item { ${values} }
    ?item p:P54 ?mem.
    ?mem ps:P54 ?club.
    ...
    FILTER(LANG(?clubLabel)="ja" || LANG(?clubLabel)="en")
  }`
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const res = await fetch('https://query.wikidata.org/sparql', {
      ...
      for (const row of data.results.bindings) {
        const id = row.item.value.split('/').pop()
        const entry = info[id] || {}
        if (row.clubLabel && (!entry.club || row.clubLabel['xml:lang'] === 'ja')) entry.club = row.clubLabel.value
        info[id] = entry
      }
      return
    } catch {
...
for (let i = 0; i < ids.length; i += 200) {
  ...
  console.log(`  base ${Math.min(i + 200, ids.length)}/${ids.length}`)
  await sleep(200)
}
for (let i = 0; i < ids.length; i += 150) {
  await clubChunk(ids.slice(i, i + 150))
  console.log(`  club ${Math.min(i + 150, ids.length)}/${ids.length}`)
  await sleep(200)
}
...
for (const name of names) {
  ...
  if (entry.club) record.club = entry.club
...
const withClub = Object.values(sorted).filter((r) => r.club).length
...
const ts =
  ...
  `// Coverage: name ${withJa}, photo ${withPhoto}, height ${withHeight}, dob ${withDob}, club ${withClub} of ${names.length}.\n` +
...
console.log(`\nDONE -> src/data/playerInfoJa.ts | ja:${withJa} photo:${withPhoto} height:${withHeight} dob:${withDob} club:${withClub} / ${names.length}`)
